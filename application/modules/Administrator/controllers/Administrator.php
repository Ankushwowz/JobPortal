...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	public function commissionSetting(){
		$data['main_content'] = 'commission-setting';
		$data['user'] =getUser();
		$data['commission'] = $this->Administrator_model->getCommission();
		if(!empty($_POST['commissionSubmit'])){
			$this->form_validation->set_rules('commission', 'Commission', 'required');
			
			if($this->form_validation->run() === FALSE){
				$this->load->view('backend/backend-admin-layout.php', $data);
			}else{
				$result = $this->Administrator_model->commissionSetting();
				if($result){
					$this->session->set_flashdata('success', 'commission has been update successfully.');
				}
				$this->load->view('backend/backend-admin-layout.php', $data);
		...
		$data['main_content'] = 'apply-commission';
		$data['user'] =getUser();
		$data['amount'] = $this->Administrator_model->getAmountByJob();
