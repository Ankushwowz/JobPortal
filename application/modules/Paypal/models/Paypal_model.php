...
// Get Job Amount //
		$this->db->select('job.job_amount');
		$this->db->where('job_id',$getOrder['job_id']);
		$Jobquery = $this->db->get('job');
		$getJob = $Jobquery->row_array();
		//

		// Get commission//
		$this->db->select('*');
		$commissionquery = $this->db->get('commission');
		$getcommission = $commissionquery->row_array();
		//
		...
		$created = date('y-m-d h:i:s');
			$dataInsert = array(
				...
				'amount' => $amount,
				'transactionId' => $tx,
				'job_amount' =>$getJob['job_amount'],
				'percentage' =>$getcommission['commission'],
				'status' => $status,
