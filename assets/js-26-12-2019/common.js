 $(document).ready(function(){
	...
	*/
	...
	/******Auto Search *****************/
		...
		$('.enterblock').keypress(function (event) {
   ...
   /*$('#form-tags-4').tagsInput({
	...
	var uploadfilesName = $('#uploadfilesName').val();
	 var message_job_id = $('#message_job_id').val();
...
//	formData.append('chatFile', $('input[type=file]')[0].files[0]); 
	...
	formData.append('message_job_id', message_job_id); 
        ...
        $.ajax({
				...
				success:function(response){
					...
					$('#chatFile').val('');
					$('#status').html('');
					 singleChat(senderId,receiverId,message_job_id);
...
$("div .messages").scroll(function(){
   ...
   if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
	 ...
	 $('body').on('click', '.complete_job', function() {
      ...
      var ID =$(this).attr('rel');
		swal({
          ...
          if (isConfirm) {
            swal({
              title: 'Completed!',
              text: 'Job has been successfully completed!',
              icon: 'success'
            }).then(function() {
				H5_loading.show(); 
				$.ajax({
					url:base_url+"candidate/completeJob",
					method:"POST",
					data:{ID:ID,job_status:5},
					success:function(msg){
						H5_loading.hide();
	...
	$('body').on('click', '.assign_job', function() {
		 var pageURL = jQuery(location). attr("href");
      var ID =$(this).attr('rel');
		swal({
			 title: "Are you sure you want to Assign this job",
			  text: "",
			  icon: "warning",
			  buttons: [
				'No, cancel it!',
          ...
          if (isConfirm) {
            swal({
              title: 'Completed!',
              text: 'Job has been successfully assign!',
              icon: 'success'
            }).then(function() {
				H5_loading.show(); 
				$.ajax({
		...
		$("#question1 .stars").click(function () {
			var rate = $(this).val();
			$('#answer1').val(rate);
			$(this).attr("checked");
			$('#error-1').hide();
		});
		$("#question2 .stars").click(function () {
			var rate = $(this).val();
			$('#answer2').val(rate);
			$(this).attr("checked");
			$('#error-2').hide();
		});
		$("#question3 .stars").click(function () {
			var rate = $(this).val();
			$('#answer3').val(rate);
			$(this).attr("checked");
			$('#error-3').hide();
		});
		$("#question4 .stars").click(function () {
			var rate = $(this).val();
			$('#answer4').val(rate);
			$(this).attr("checked");
			$('#error-4').hide();
	...
	$('body').on('click', '#submit_review', function() {
		...
		var general_feedback =$('#general_feedback').val();
		var to_id =$('#to_id').val();
		...
		if(general_feedback==''){
			$('#general_feedback').css('border-color','red');
			return false;
		}
		
		$.ajax({
			url:base_url+"candidate/sendRatting",
			method:"POST",
			data:{
			answer1:answer1,
			answer2:answer2,
			...
			success:function(response){
				if(response==1){
					window.location.href= base_url+'completed-job';
				}
			}
		});
			...
			$('body').on('click', '.resend_Job', function() {
		 var pageURL = jQuery(location). attr("href");
      var ID =$(this).attr('rel');
		swal({
			 title: "Are you sure you want to resend this job",
			  text: "",
			  icon: "warning",
			  buttons: [
				'No, cancel it!',
          ...
          if (isConfirm) {
            swal({
              title: 'Completed!',
              text: 'Job has been successfully resend!',
              icon: 'success'
            }).then(function() {
				H5_loading.show(); 
				$.ajax({
					url:base_url+"candidate/resendJob",
					method:"POST",
					data:{ID:ID,job_status:4},
					success:function(msg){
						H5_loading.hide();
						window.location.href= pageURL;
					}
		...
		$('body').on('change', '#change_job_status', function() {
		 var pageURL = jQuery(location). attr("href");
		var str =$(this).val();
		...
		var res = str.split("_");
		swal({
			 title: "Are you sure you want to assigned again this job",
			  text: "",
			  icon: "warning",
			  buttons: [
				'No, cancel it!',
          ...
          if (isConfirm) {
            swal({
              title: 'Completed!',
              text: 'Job has been successfully assigned again!',
              icon: 'success'
            }).then(function() {
				H5_loading.show(); 
				$.ajax({
					url:base_url+"candidate/assigned_again",
					method:"POST",
					data:{ID:res[1],job_status:res[0]},
					success:function(msg){
						H5_loading.hide();
						window.location.href= pageURL;
					}
		...
		$('body').on('click', '.assign_job_cls', function() {
		 var pageURL = jQuery(location). attr("href");
          ...
          if (isConfirm) {
            swal({
              ...
              icon: 'success'
				...
				H5_loading.show(); 
				$.ajax({
					url:base_url+"candidate/assign_job_by_response",
					method:"POST",
					data:{UserId:UserId,job_id:job_id,job_status:3},
					success:function(msg){
						H5_loading.hide();
						window.location.href= pageURL;
					}
...
function giveRatting(inviteID,starRatting){
	var rate = starRatting;
	alert(starRatting);
	var start_value = $('#start_value_'+inviteID).val(rate);
		//$(this).attr("checked");
		return false;
}
...
function replyUserByJob(userId,jobId,userName,apply_job_id){
	$('#user_id').val(userId);
	$('#job_id').val(jobId);
	$('#userName').text(userName);
    $('#apply_job_id').val(apply_job_id);
	
}



function singleChat(senderId,receiverId,jobid){
	$('#receiverId').val(receiverId);
	$('#message_job_id').val(jobid);
	...
	$.ajax({
				...
				method:"POST",
				data:{senderId:senderId,receiverId:receiverId,userName:userName,message_job_id:jobid},
				success:function(response){
				    H5_loading.hide(); 
					 var returnedData = JSON.parse(response);
					...
					success:function(response){
						var returnedData = JSON.parse(response);
						 ...
						 $('.messagecount').replaceWith(returnedData.UnreadCount);
						  ...
						  $('.assignedJobcount').replaceWith(returnedData.UnreadAssignedJob);
							$('.invitationJob').replaceWith(returnedData.InvitationJob);
							 $('.inviteuser').replaceWith(returnedData.inviteuser);
							  $('.completedJobcount').replaceWith(returnedData.CompletedJob);
						   scrollBottom();
					}
			});
...
function getUserInfo(receiverId){
	...
	$.ajax({
				...
				method:"POST",
				data:{receiverId:receiverId,message_job_id:message_job_id},
				success:function(response){
...
function uploadFile() {
	var base_url =$('#base_url').val();
  var file = _("userfile_id").files[0];
 // alert(file.name+" | "+file.size+" | "+file.type);
  if(file.size > 8289384){
  	$('#sizeerror').show();
  	return false;
  }
  var formdata = new FormData();
  ...
  ajax.open("POST", base_url+"job/uploadfiles"); // http://www.developphp.com/video/JavaScript/File-Upload-Progress-Bar-Meter-Tutorial-Ajax-PHP
...
function uploadmessageFile() {
 ...
 // alert(file.name+" | "+file.size+" | "+file.type);
  ...
  ajax.addEventListener("error", errorHandler, false);
  ajax.addEventListener("abort", abortHandler, false);
  ajax.open("POST", base_url+"message/uploadfiles"); // http://www.developphp.com/video/JavaScript/File-Upload-Progress-Bar-Meter-Tutorial-Ajax-PHP
  //use file_upload_parser.php from above url
  ajax.send(formdata);
}
...
function completeHandler(event) {
  ...
  _("uploadfilesName").value = event.target.responseText;
